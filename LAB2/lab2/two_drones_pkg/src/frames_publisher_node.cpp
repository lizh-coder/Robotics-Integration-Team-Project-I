...
class FramesPublisherNode {
 ...
 public:
  ...
  void onPublish(const ros::TimerEvent&) {
    ...
    // The tangent of [cos(t), sin(t)] is [-sin(t), cos(t)].  Therefore
    // yaw=t makes the body y-axis tangent while keeping z parallel to world z.
    tf2::Quaternion av1_rotation;
    av1_rotation.setRPY(0.0, 0.0, time);
    AV1World.transform.rotation = tf2::toMsg(av1_rotation);
